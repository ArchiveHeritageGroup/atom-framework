...
class QubitMenu extends BaseMenu
{
    ...
    public function getPath($options = [])
    {
        ...
        if (isset($options['getUrl']) && true == $options['getUrl']) {
            ...
            try {
                ...
                // Name the homepage route rather than asking for a blank one.
                // url_for('') generates the FIRST route in the collection, which is
                // the homepage only for as long as nothing is registered ahead of it.
                // Any plugin that registers routes early takes that slot, and every
                // unresolvable menu path then points at whatever it happens to be -
                // '/admin/pricing' with ahgCartPlugin enabled, '/admin/share-links'
                // with ahgShareLinkPlugin. Base AtoM was relying on registration
    ...
    public function checkUserAccess()
    {
        ...
        // A fragment or an external URL has no module behind it to authorise - the
        // clipboard's 'Clear all selections' is handled in JavaScript. findRoute()
        // answers null for those, and reading ['parameters'] off null is fatal, so
        // the entire page dies rather than the one menu entry. Show it: there is no
        // action to gate.
        if (!isset($route['parameters']['module'], $route['parameters']['action'])) {
            return true;
        }
