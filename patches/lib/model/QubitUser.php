...
class QubitUser extends BaseUser
{
    ...
    public function setPassword($password)
    {
        ...
        // SECURITY: use a cryptographically secure random salt (was a predictable
        // md5(rand(100000,999999) . email), a tiny seed space keyed on the email).
    ...
    public static function checkCredentials($username, $password, &$error)
    {
        ...
        if (null !== $user) {
            ...
            // Verify via the framework's scheme-aware PasswordService: legacy
            // password_verify(sha1(salt.plaintext)) when the stored salt is
            // non-empty, Argon2id-over-plaintext when empty (2026-06-15 migration).
            // Falls back to the legacy inline check if the service is unavailable.
            $passwordOk = class_exists('\\AtomFramework\\Core\\Security\\PasswordService')
                ? \AtomFramework\Core\Security\PasswordService::verify($password, (string) $user->getPasswordHash(), $user->getSalt())
