...
class QubitPhysicalObjectCsvHoldingsReport
{
    public static $headerRow = [
        ...
        'physicalObjectType',
        // heratio#145 — AHG strongroom assignment, slotted between physical-
        // object cols and holding cols so downstream consumers that index by
        // header name (rather than position) keep working.
        'physicalObjectStrongroom',
        'physicalObjectStrongroomSizeUsed',
        'holdingType',
        'holdingIdentifier',
        'holdingTitle',
        'levelOfDescription',
    ...
    public function writeRowIfNecessary(object $writer, object $physObject, array $holdingsData)
    {
        ...
        // heratio#145 — Append AHG strongroom assignment columns. No-op on
        // vanilla AtoM (the ahg_physical_object_storage table is absent).
        $row = self::appendStrongroomColumns($row, (int) $physObject->id);
    ...
    /**
     * heratio#145 — Append the AHG strongroom assignment columns to a row
     * (name + size-used-with-unit). Fails soft when the AHG tables are absent
     * (e.g. on a vanilla AtoM install): both columns come back as empty
     * strings. Static so the call site doesn't need an instance.
     */
    public static function appendStrongroomColumns(array $row, int $physicalObjectId): array
    {
        try {
            $sql = 'SELECT sr.name AS name, ps.size_units_used AS size_used, sr.capacity_unit AS unit
                    FROM ahg_physical_object_storage ps
                    INNER JOIN ahg_strongroom sr ON sr.id = ps.strongroom_id
                    WHERE ps.physical_object_id = ?';
            $rows = QubitPdo::fetchAll($sql, [$physicalObjectId], ['fetchMode' => PDO::FETCH_ASSOC]);
            if (!empty($rows)) {
                $r = $rows[0];
                $row[] = (string) ($r['name'] ?? '');
                $sizeUsed = $r['size_used'] ?? '';
                $unit = $r['unit'] ?? '';
                $row[] = ('' !== $sizeUsed && '' !== $unit)
                    ? $sizeUsed.' '.$unit
                    : (string) $sizeUsed;
            ...
            // ahg tables absent / db error — fall through to empty values.
        } catch (Throwable $e) {
        }
        $row[] = '';
        $row[] = '';
