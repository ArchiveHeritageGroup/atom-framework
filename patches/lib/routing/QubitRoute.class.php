        ...
        if (isset($params[0])) {
            ...
            foreach (array_diff_key($this->params + $this->variables, $params) as $key => $ignore) {
                try {
                    ...
                    // PHP 8 (ATOM-11, AHG 2026-08-25).
                    //
                    // The catch above was written when reading a string offset
                    // with a string key was a warning that yielded ''. PHP 8
                    // made it a TypeError - "Cannot access offset of type
                    // string on string" - which is not an sfException, so it
                    // passes straight through this guard and out of the
                    // request uncaught.
                    //
                    // $params[0] is a string whenever url_for() is handed a
                    // path instead of a resource, which is reachable from an
                    // unauthenticated POST to any unroutable path. An external
                    // scanner POSTing /admin, /login, /register and the like
                    // threw this 31 times in 2.5 minutes on PSIS, 2026-08-25.
                    //
                    // Worse than the noise: these arrive with no status code,
                    // so they take the set_exception_handler path in
                    // ErrorNotificationService, which ALSO sends an alert
                    // email. The 404 suppression added in v3.99.31/32 does not
                    // cover it, because a TypeError is not an
                    // sfError404Exception. So a remote scanner could drive the
                    // error-alert mail.
                    //
                    // The original intent is unchanged: a key that cannot be
                    // read is skipped. Only the exception class differs.
                }
            }
