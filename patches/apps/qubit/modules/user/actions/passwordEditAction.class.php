    ...
    public function execute($request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                $plainPassword = trim($this->form->getValue('password'));

                // Check password reuse before saving
                $reuseBlocked = false;
                if (strlen($plainPassword) > 0 && $this->resource->id) {
                    try {
                        // Use unsalted SHA1 for consistent history comparison
                        $sha1ForCheck = sha1($plainPassword);
                        if (\AtomFramework\Core\Security\PasswordPolicyService::isPasswordReused(
                            $this->resource->id, $sha1ForCheck
                        )) {
                            $this->form->getErrorSchema()->addError(new sfValidatorError(
                                new sfValidatorPass(),
                                $this->context->i18n->__('This password has been used recently. Please choose a different password.')
                            ));
                            $reuseBlocked = true;
                        }
                    } catch (\Throwable $e) {
                ...
                if (!$reuseBlocked) {
                    ...
                    if (strlen($plainPassword) > 0 && $this->resource->id) {
                        try {
                            $sha1ForHistory = sha1($plainPassword);
                            $hashForHistory = QubitUser::generatePasswordHash($sha1ForHistory);
                            \AtomFramework\Core\Security\PasswordPolicyService::recordPasswordChange(
