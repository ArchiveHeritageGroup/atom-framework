...
class InformationObjectExportCsvAction extends sfAction
{
    ...
    public function execute($request)
    {
        if ($this->context->user->isAuthenticated()) {
            ...
            // SECURITY: non-editors must not export unpublished descriptions.
            // Force the publication-status filter to "published" unless the user
            // holds the editor credential (i.e. may view drafts).
            if (!$this->context->user->hasCredential('editor')) {
                $getParameters['publicationStatus'] = QubitTerm::PUBLICATION_STATUS_PUBLISHED_ID;
            }
