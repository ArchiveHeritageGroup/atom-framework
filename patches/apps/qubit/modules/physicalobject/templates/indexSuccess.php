...
<?php
// heratio#145 — AHG strongroom assignment. Read-only display; assignment is
// managed from /strongroom/<slug>. Fails soft when the AHG tables are absent.
try {
    $srRow = QubitPdo::fetchOne(
        'SELECT sr.slug AS slug, sr.name AS name FROM ahg_physical_object_storage ps'
        .' INNER JOIN ahg_strongroom sr ON sr.id = ps.strongroom_id'
        .' WHERE ps.physical_object_id = ?',
        [$resource->id]
    );
    if ($srRow) {
        $srLink = link_to($srRow->name, ['module' => 'strongroom', 'action' => 'show', 'slug' => $srRow->slug]);
        echo render_show(__('Strongroom'), $srLink, ['valueClass' => 'field', 'renderAsIs' => true]);
    }
