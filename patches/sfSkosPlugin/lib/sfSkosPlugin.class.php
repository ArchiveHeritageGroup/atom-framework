...
class sfSkosPlugin
{
    ...
    public function importGraph()
    {
        ...
        //
        // SECURITY/ROBUSTNESS (ATOM-10, AHG 2026-08-24): this loop and the one
        // above disagree about which concepts exist, and the mismatch was fatal.
        //
        // addConcept() stamps atom:id only on concepts it imports, and it is
        // reached only for a root concept or something below one via
        // skos:narrower. This loop walks EVERY skos:Concept in the graph, so a
        // concept the first pass never saw has no atom:id, get() returns null,
        // and ->getValue() on it killed the whole import with
        // "Call to a member function getValue() on null" - after earlier terms
        // had already been saved, so a retry duplicated them.
        //
        // getRootConcepts() is what makes it easy to hit: with no
        // skos:ConceptScheme every concept is imported and this cannot happen,
        // but a file declaring a scheme with skos:hasTopConcept narrows the
        // first pass to those top concepts. Adding well-formed scheme metadata
        // to a working file was enough to break it.
        //
        // Either end can be the null one - the subject too, since this loop
        // iterates concepts the first pass never visited.
        //
        // Skip and log rather than fail: a relation that cannot be expressed is
        // not a reason to lose the terms, and this matches how the method above
        // already reports a concept of an unexpected type.
