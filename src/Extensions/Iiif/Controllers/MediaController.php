...
class MediaController
{
    ...
    public function getMetadata(int $id): void
    {
        ...
        // Build response with basic info + any extended metadata
        $response = [
            'digital_object_id' => $id,
            'filename' => $do->name,
            'mime_type' => $do->mime_type,
            'byte_size' => $do->byte_size,
            'path' => $do->path,
        ];
        ...
        if ($meta) {
            ...
            $response['bitrate'] = $meta->bitrate;
            $response['format'] = $meta->format;
            $response['width'] = $meta->video_width;
            $response['height'] = $meta->video_height;
            $response['codec'] = $meta->video_codec ?: $meta->audio_codec;
            $response['video_codec'] = $meta->video_codec;
            $response['frame_rate'] = $meta->video_frame_rate;
            $response['audio_codec'] = $meta->audio_codec;
            $response['sample_rate'] = $meta->audio_sample_rate;
            $response['channels'] = $meta->audio_channels;

            // Parse EXIF/XMP if stored as JSON
            if (!empty($meta->raw_metadata)) {
                $rawMeta = json_decode($meta->raw_metadata, true);
                if ($rawMeta) {
                    $response['exif'] = $rawMeta['exif'] ?? null;
                    $response['xmp'] = $rawMeta['xmp'] ?? null;
                    $response['iptc'] = $rawMeta['iptc'] ?? null;
                }
            }
        }
