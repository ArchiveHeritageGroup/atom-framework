...
if (!defined('ATOM_ROOT_PATH')) {
    // Find the AtoM root by looking for config/config.php, rather than assuming
    // how deep this file sits.
    //
    // This file ships from two locations: as atom-framework/bootstrap.php at the
    // AtoM root, and generated by bin/build-runtime-plugin into
    // plugins/ahgRuntimePlugin/bootstrap.php, one directory deeper. A hardcoded
    // dirname(__DIR__) is right for the first and silently wrong for the second -
    // it resolves to <root>/plugins, atomParseDbConfig() finds no config.php and
    // returns null, and the Capsule below is never created. Nothing is thrown and
    // nothing is logged; the first symptom is every query dying with
    // "Call to a member function connection() on null", far from the cause.
    $atomRoot = null;
    $candidate = __DIR__;

    for ($depth = 0; $depth < 6; ++$depth) {
        if (file_exists($candidate.'/config/config.php')) {
            $atomRoot = $candidate;
    ...
    // Fall back to the historical behaviour rather than guessing at '/'.
    define('ATOM_ROOT_PATH', $atomRoot ?? dirname(__DIR__));

    unset($atomRoot, $candidate, $parent, $depth);
}
