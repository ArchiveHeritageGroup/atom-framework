#!/bin/bash
#===============================================================================
# AtoM Setup Wizard v2.1
# Interactive installer following official Artefactual documentation
# https://www.accesstomemory.org/docs/latest/admin-manual/installation/
#===============================================================================
...
check_requirements() {
    ...
    # RAM Check (minimum 4GB, recommended 7GB)
    ...
    if [ $errors -gt 0 ]; then
        ...
        report+="Installation may fail or perform poorly."
    elif [ $warnings -gt 0 ]; then
        report+="WARNINGS: $warnings\n\n"
        report+="System meets minimum but not recommended specs.\n"
        report+="Installation will proceed."
    else
...
#===============================================================================
if [ "$SVC_ES" = "on" ]; then
    # Calculate recommended heap (25% of RAM, max 1GB for small systems)
    local total_ram_mb=$(free -m | awk '/^Mem:/{print $2}')
    local rec_heap=$((total_ram_mb / 4))
...
if [ "$SVC_PHP" = "on" ]; then
    ...
    # Add PHP repository for older Ubuntu
    if ! apt-cache show php8.3 &>/dev/null; then
        add-apt-repository -y ppa:ondrej/php >>$LOG 2>&1
        apt-get update -qq >>$LOG 2>&1
    fi
    
    # Install PHP with all required extensions per Artefactual docs
    apt-get install -y \
...
if [ "$SVC_MYSQL" = "on" ]; then
    ...
    # Configure MySQL modes per Artefactual docs
    echo 20
...
#---------------------------------------------------------------------------
# Elasticsearch 7.10 (OSS version per Artefactual recommendation)
#---------------------------------------------------------------------------
if [ "$SVC_ES" = "on" ]; then
    echo 30
    ...
    # Add Elasticsearch OSS repository (7.x)
...
#---------------------------------------------------------------------------
echo 82
echo "XXX"; echo "Creating AtoM configuration..."; echo "XXX"
...
#---------------------------------------------------------------------------
echo 85
echo "XXX"; echo "Setting file permissions..."; echo "XXX"
...
WORKER
    ...
    sed -i "s|/usr/share/nginx/atom|$ATOM_PATH|g" /usr/lib/systemd/system/atom-worker.service
...
INSTALL_CMD="php symfony tools:install"
INSTALL_CMD+=" --database-host=localhost"
INSTALL_CMD+=" --database-port=3306"
INSTALL_CMD+=" --database-name=$DB_NAME"
INSTALL_CMD+=" --database-user=$DB_USER"
INSTALL_CMD+=" --database-password=$DB_PASS"
INSTALL_CMD+=" --search-host=localhost"
INSTALL_CMD+=" --search-port=9200"
INSTALL_CMD+=" --search-index=atom"
INSTALL_CMD+=" --site-title=\"$SITE_TITLE\""
INSTALL_CMD+=" --site-description=\"$SITE_DESC\""
INSTALL_CMD+=" --site-base-url=\"$SITE_URL\""
INSTALL_CMD+=" --admin-email=\"$ADMIN_EMAIL\""
INSTALL_CMD+=" --admin-username=\"$ADMIN_USER\""
INSTALL_CMD+=" --admin-password=\"$ADMIN_PASS\""
INSTALL_CMD+=" --no-confirmation"

if [ "$LOAD_DEMO" = "on" ]; then
    INSTALL_CMD+=" --demo"
fi

sudo -u www-data $INSTALL_CMD >>$LOG 2>&1 || {
...
if [ "$SETUP_WORKER" = "on" ]; then
    echo 98
    echo "XXX"; echo "Starting atom-worker service..."; echo "XXX"
...
#---------------------------------------------------------------------------
echo 99
echo "XXX"; echo "Verifying services..."; echo "XXX"

systemctl enable nginx php8.3-fpm mysql gearman-job-server 2>/dev/null || true
systemctl start nginx php8.3-fpm mysql gearman-job-server 2>/dev/null || true

if [ "$SVC_ES" = "on" ] || curl -s http://localhost:9200 &>/dev/null; then
